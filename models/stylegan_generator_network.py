...
class FirstConvBlock(nn.Module):
  ...
  def __init__(self,
               ...
               channels,
               w_space_dim=512,
               randomize_noise=False,get_style=False):
    super().__init__()
    ...
    self.epilogue = EpilogueBlock(resolution=init_resolution,
                                  ...
                                  randomize_noise=randomize_noise,get_style=self.get_style)

  def forward(self, w):
    x = self.const.repeat(1, 1, 1, 1)
    if self.get_style== False:
      x = self.epilogue(x, w)
      return x
    else:
      x,style = self.epilogue(x, w)
      return x,style

class StyleModulationLayer(nn.Module):
  """Implements the style modulation layer."""

  def __init__(self, channels, w_space_dim=512,get_style=False):
    super().__init__()
    self.get_style = get_style
  ...
  def forward(self, x, w):
    if len(w.size())==2:
      ...
      style = w.view(-1, 2, self.channels, w.size()[-1], w.size()[-1])

    if self.get_style==False:
      return x * (style[:, 0] + 1) + style[:, 1]
    else:
      return x * (style[:, 0] + 1) + style[:, 1],style
...
class StyleGANGeneratorNet(nn.Module):
  ...
  def __init__(self,
    ...
    self.synthesis = SynthesisModule(init_resolution=self.init_res,
                                     ...
                                     fmaps_max=self.fmaps_max,get_style=self.get_style)
      ...
      if res == self.init_res:
        ...
        self.add_module(
            ...
            UpConvBlock(resolution=res,
                        ...
                        fused_scale=fused_scale,get_style=self.get_style))
  ...
  def __init__(self,
    ...
    if normalization_fn == 'pixel':
      ...
      raise NotImplementedError(f'Not implemented normalization function: '
                                f'{normalization_fn}!')
    self.style_mod = StyleModulationLayer(channels, w_space_dim=w_space_dim,get_style=self.get_style)
