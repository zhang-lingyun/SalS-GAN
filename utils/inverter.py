...
class StyleGANInverter(object):
  ...
  def get_init_code(self, image,sizes):
    ...
    init_code = _get_tensor_value(self.E.net(x).view(1, *self.encode_dim)).astype(np.float32) # 1,14,512

    ############################ get alpha,beta from generator
    tmp_G = StyleGANGenerator(self.model_name, self.logger,get_style=True)
    tmp_init_code = torch.Tensor(init_code).to(self.run_device)
    tmp_latents = []
    for i in range(tmp_init_code.size()[1]):
      tmp_latents.append(tmp_init_code[:,i])

    img,init_mat = tmp_G.net.synthesis(tmp_latents)
    del tmp_G
    ############################
    ...
    for idx,size in enumerate(sizes):
      if size == (1,512):
        ...
        z[:] = init_mat[idx].detach().cpu()

      z = torch.Tensor(z).to(self.run_device)
      z.requires_grad = True
      latents.append(z)
    
    return latents

  def invert(self, image,sizes, num_viz=0):
    ...
    latents = self.get_init_code(image,sizes)
    ...
    x_init_inv = self.G.net.synthesis(latents)
    ...
    for step in pbar:
      ...
      x_rec = self.G.net.synthesis(latents)

      # Reconstruction loss.
      loss_pix = torch.mean((x - x_rec) ** 2)
      loss = loss + loss_pix * self.loss_pix_weight
      log_message = f'loss_pix: {_get_tensor_value(loss_pix* self.loss_pix_weight):.3f}'
      ...
      if self.loss_feat_weight:
        ...
        log_message += f', loss_feat: {_get_tensor_value(loss_feat * self.loss_feat_weight):.3f}'
      ...
      if self.loss_reg_weight:
        ...
        cnt = 1
        for i in range(14):
          if latents[i].shape != (1,512):
            continue
          loss_reg += torch.mean((latents[i].squeeze() - z_rec[0,i].squeeze()) ** 2)
          cnt += 1

        loss = loss + loss_reg/cnt * self.loss_reg_weight
    ...
    return latents, viz_results

  def easy_invert(self, image,sizes, num_viz=0):
    """Wraps functions `preprocess()` and `invert()` together."""
    return self.invert(self.preprocess(image), sizes,num_viz)
